...
void wk_free(WKBox *a) {
	...
	if (first_run) {
		first_run = false;
		wk_free_bus_connect("int", NULL);
		wk_free_bus_connect("size_t", NULL);
		wk_free_bus_connect("void *", NULL);
		wk_free_bus_connect("float", NULL);
		wk_free_bus_connect("double", NULL);
		wk_free_bus_connect("const char *", NULL);
	}
...
size_t wk_hash(WKBox *a) {
	...
	if (first_run) {
		first_run = false;
		wk_hash_bus_connect("int", NULL);
		wk_hash_bus_connect("size_t", NULL);
		wk_hash_bus_connect("void *", NULL);
		wk_hash_bus_connect("float", NULL);
		wk_hash_bus_connect("double", NULL);		
	}
        for (size_t i = 0; i < _wk_hash_bus_->n; i++) {
                WKHashSlot *slot = wk_array_get(_wk_hash_bus_, i, WKHashSlot *);
		if (wk_box_is(a, slot->type)) {
			if (slot->action) return slot->action(a);
			else { /* 处理基本类型 */
				return wk_box_get(a, size_t);
			}
		}
        }
	wk_err("unknown type!");
